...
Porté depuis planexpert-core/releve/render_pdf.py (imports relatifs de
package ; dépendance `markdown` importée paresseusement, uniquement pour
md_to_pdf).
...
def annotate_sheet(work, f, info, occ_f, nom):
    ...
    if counts:
        ...
        dr.text((x0 + 15, y0 + 10), f"Légende — {f} ({sum(counts.values())} marques)", fill=(0, 0, 0, 255), font=font(22))
...
def md_to_pdf(md_text):
    import markdown   # dépendance optionnelle, requise seulement ici
...
def main(work, name, out_dir):
    ...
    L = [f"# Rapport de métré (par plans) — {name}", "", f"Généré le {today} par le pipeline `src.pipeline` (relevé automatique + scripts déterministes). "
    ...
    for f in sheets:
        L.append(f"| {f} | {sum(per_sheet[f].values())} | {len(per_sheet[f])} |")
    ...
    for f in sheets:
        if not per_sheet[f]:
            continue
        L += ["", f"## {f}", "", "| Libellé | Ce plan | Tous les plans |", "|---|--:|--:|"]
