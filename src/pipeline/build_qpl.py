...
Porté depuis planexpert-core/releve/build_qpl.py (imports relatifs de package).
Pour un .qpl riche de toutes les options (légende positionnée, lignes,
calques nommés), préférer `python -m src.releve qpl` (qpl_build.py).

Usage : python -m src.pipeline.build_qpl WORKDIR NOM_PROJET SORTIE_DIR
Produit SORTIE_DIR/<NOM_PROJET>.qpl et copie les rasters <feuille>.png à côté (Plan Expert les référence en relatif).
Format XML observé sur les fichiers natifs Plan Expert 3.0.17 (voir docs/pipeline-qpl.md §2) :
...
def main(work, name, out_dir):
    ...
    L.append(f'\t\t<ActivePlan Name="{q(marked[0] if marked else order[0])}"/>')
    L.append("\t\t<RecentPlans>")
    for f in marked[:10]:
        L.append(f'\t\t\t<Plan Name="{q(f)}"/>')
    ...
    for f in order:
        ...
        L += [f'\t\t<Plan Name="{q(f)}" FileName="{q(f)}.png">', f'\t\t\t<Thumbnail FileName="{uuid.uuid4()}"/>', "\t\t\t" + scale,
