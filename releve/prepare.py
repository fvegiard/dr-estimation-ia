...
Usage : uv run releve/prepare.py INBOX_DIR WORKDIR
    ...
    for path in files:
        ...
        if kind in ("plans", "addenda"):
            for i, page in enumerate(doc):
                ...
                if sid is None:
                    # Aucune page n'est ignorée (audit S-1769/S-1714 du 2026-09-22 : 5/7 et 34/44 pages perdues) :
                    # la feuille reçoit un nom provisoire <fichier>-pNN et l'agent lit le vrai numéro dans le cartouche (aperçu).
                    stem = re.sub(r"[^A-Za-z0-9]+", "", os.path.splitext(os.path.basename(rel))[0])[:10] or "PLAN"
                    sid = f"{stem}-p{i + 1:02d}"
                    notes.append(f"{rel} page {i + 1} : aucun numéro de feuille lu dans le cartouche — feuille nommée provisoirement {sid}, "
                                 "numéro réel à lire sur l'aperçu (cartouche) par l'agent")
                if kind == "addenda":
                    sid = f"{sid}_ADD"
                seen_sheets[sid] += 1
...
def prepare_sheet(doc, i, fid, rel, kind, work):
    ...
    # Pages tournées (Rotate 90/270 : S-1714, S-1769, S-1797, S-1811…) : get_text() renvoie des coordonnées NON tournées
    # alors que rasters et tuiles suivent page.rect (tournée). On ramène chaque mot dans le repère tourné (audit 2026-09-22).
    M = page.rotation_matrix
    words = []
    for x0, y0, x1, y1, t, *_ in page.get_text("words"):
        r = pymupdf.Rect(x0, y0, x1, y1) * M
        words.append((r.x0, r.y0, r.x1, r.y1, t))
